...
TripTrackForm::OnInitializing(void)
{
	...
	__pRegionCenterLatitude->Construct(Rectangle(wMargin, y, fieldWidth, fieldHeight), EDIT_FIELD_STYLE_IP_V4, INPUT_STYLE_FULLSCREEN, true);
	...
	__pRegionCenterLongitude->Construct(Rectangle(wMargin, y, fieldWidth, fieldHeight), EDIT_FIELD_STYLE_IP_V4, INPUT_STYLE_FULLSCREEN, true);
	__pRegionCenterLongitude->SetTitleText(L"Center Longitude");

	y = y + fieldHeight + hMargin;

	__pRegionRadius = new (std::nothrow) EditField();
	__pRegionRadius->Construct(Rectangle(wMargin, y, fieldWidth, fieldHeight), EDIT_FIELD_STYLE_IP_V4, INPUT_STYLE_FULLSCREEN, true);
	...
	Tizen::Ui::Controls::Button* pButtonOk = new (std::nothrow) Button();
	pButtonOk->Construct(Rectangle(wMargin, y, buttonWidth, buttonHeight), L"OK");
	pButtonOk->SetActionId(ID_POPUP_REGION_OK);
	pButtonOk->AddActionEventListener(*this);

	Tizen::Ui::Controls::Button* pButtonCancel = new (std::nothrow) Button();
	pButtonCancel->Construct(Rectangle((int)popupClientArea.width/2 + wMargin, y, buttonWidth, buttonHeight), L"Cancel");
	pButtonCancel->SetActionId(ID_POPUP_REGION_CANCEL);
	pButtonCancel->AddActionEventListener(*this);

	__pRegionAddPopup->AddControl(*pButtonOk);
	__pRegionAddPopup->AddControl(*pButtonCancel);
...
TripTrackForm::OnActionPerformed(const Tizen::Ui::Control& source, int actionId)
{
	switch(actionId)
		...
		case ID_FOOTER_REMOVE_MONITORING_REGION:
		{
			...
			if (r == E_INVALID_ARG)
			{
				ShowMessageBox(L"Invalid operation", L"Region cannot be removed due to invalid region Id.");
			}
			else
			{
